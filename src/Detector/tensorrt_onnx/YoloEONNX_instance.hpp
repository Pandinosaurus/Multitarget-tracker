...
///
/// \brief The YOLOE_onnx class
///
class YOLOE_instance_onnx : public YoloONNX
{
public:
	YOLOE_instance_onnx(std::vector<std::string>& inputTensorNames, std::vector<std::string>& outputTensorNames)
	{
...
protected:
	...
	std::vector<tensor_rt::Result> GetResult(size_t imgIdx, int /*keep_topk*/, const std::vector<float*>& outputs, cv::Size frameSize)
	{
		...
		cv::Rect frameRect(0, 0, INPUT_W, INPUT_H);
...
//
				...
				if (width > 4 && height > 4)
				{
					...
					rectBoxes.back() = rectBoxes.back() & frameRect;
