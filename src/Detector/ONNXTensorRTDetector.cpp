...
/// \param colorFrame
///
ONNXTensorRTDetector::ONNXTensorRTDetector(const cv::UMat& colorFrame)
    : BaseDetector(colorFrame)
{
	m_localConfig.m_calibrationImageListFileTxt = "";
	m_localConfig.m_inferencePrecision = tensor_rt::FP32;
	m_localConfig.m_netType = tensor_rt::YOLOV4;
	m_localConfig.m_detectThresh = 0.5f;
	m_localConfig.m_gpuInd = 0;
}
...
/// \param colorFrame
///
ONNXTensorRTDetector::ONNXTensorRTDetector(const cv::Mat& colorFrame)
    : BaseDetector(colorFrame)
{
	m_localConfig.m_calibrationImageListFileTxt = "";
	m_localConfig.m_inferencePrecision = tensor_rt::FP32;
...
///
/// \brief ONNXTensorRTDetector::Init
/// \return
///
bool ONNXTensorRTDetector::Init(const config_t& config)
{
	...
	if (confidenceThreshold != config.end())
		m_localConfig.m_detectThresh = std::stof(confidenceThreshold->second);

	auto gpuId = config.find("gpuId");
	if (gpuId != config.end())
		m_localConfig.m_gpuInd = std::max(0, std::stoi(gpuId->second));

	auto maxBatch = config.find("maxBatch");
	if (maxBatch != config.end())
        m_batchSize = std::max(1, std::stoi(maxBatch->second));
	m_localConfig.m_batchSize = static_cast<uint32_t>(m_batchSize);

    auto videoMemory = config.find("video_memory");
    if (videoMemory != config.end())
        m_localConfig.m_videoMemory = std::max<size_t>(0, std::stoul(videoMemory->second));

    m_localConfig.m_fileModelCfg = modelConfiguration->second;
    m_localConfig.m_fileModelWeights = modelBinary->second;
	...
	if (net_type != config.end())
	{
		...
		if (netType != dictNetType.end())
			m_localConfig.m_netType = netType->second;
		else
		{
			assert(netType == dictNetType.end());
			std::cerr << "net_type = " << net_type->second << ", " << (int)m_localConfig.m_netType << std::endl;
		}
	}
...
///
/// \brief ONNXTensorRTDetector::Detect
/// \param gray
///
void ONNXTensorRTDetector::Detect(const cv::UMat& colorFrame)
{
    m_regions.clear();
...
/// \param regions
///
void ONNXTensorRTDetector::Detect(const std::vector<cv::UMat>& frames, std::vector<regions_t>& regions)
{
    if (frames.size() == 1)
    {
        Detect(frames.front());
...
/// \param frame
///
void ONNXTensorRTDetector::CalcMotionMap(cv::Mat& frame)
{
